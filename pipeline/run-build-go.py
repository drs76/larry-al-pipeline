#!/usr/bin/env python3
"""
Larry Go build orchestrator — Pi agent harness, local, free.

Go analogue of run-build.py (which is AL/dotnet-specific). Same keep-best /
no-regress fix loop and pi->Claude escalation, but the referee is the Go
toolchain instead of the AL compiler.

Flow:
  1. Larry (Pi) writes files from the handover (larry-handover.prompt.md)
  2. Per round: gofmt -w . + go mod tidy, then go build ./... -> go vet ./...
     -> go test ./... (only if the project has *_test.go). Toolchain = ground truth.
  3. Errors -> Larry (Pi) fixes -> re-run. Keep-best across up to MAX_FIX_ROUNDS.
  4. Escalation: pi is free/local; after ESCALATE_AFTER rounds hand the rest to
     Claude Code (billed). OFF unless ESCALATE_AFTER=N is set.

Usage:
  python3 run-build-go.py --project <root>   # target a Go project (module) root
  python3 run-build-go.py --project <root> --skip-larry    # fix existing files only
  python3 run-build-go.py --project <root> --review-only   # just the referee loop

Model: env PI_CODER_MODEL (default ollama/ornith:35b). Go correctness rules
live in the handover. Expected files come from an optional 'machine-readable
manifest' block; if absent, "at least one .go file exists" is the write gate.
"""
...
# Coder backend: "pi" (local ollama, free) or "claude" (Claude Code, sub quota)
...
# Claude Code (first-party). NOTE: correct path is ~/.local/bin/claude on this box
# (run-build.py hardcodes the wrong npm path — a known AL-side bug).
...
# Escalation: after ESCALATE_AFTER pi fix rounds without a pass, latch to Claude.
# OFF by default so paid quota is never spent unless you opt in.
...
# Per-project paths — set by configure_project().
PROJECT_ROOT   = None
HANDOVER       = None
EXPECTED_FILES = []
...
def parse_manifest(handover_path):
    """Extract absolute file paths from the handover's final fenced block under
    a 'machine-readable manifest' heading. Optional for Go — returns [] if none."""
    try:
...
def configure_project(root):
    ...
    if EXPECTED_FILES:
        ...
        print("  No manifest — write gate = '>=1 .go file exists'.")
...
def go_files(include_tests=True):
    out = []
    for dp, dirs, fs in os.walk(PROJECT_ROOT):
        dirs[:] = [d for d in dirs if d not in ("vendor", ".git")]
        for fn in fs:
            if fn.endswith(".go") and (include_tests or not fn.endswith("_test.go")):
                out.append(os.path.join(dp, fn))
    return out


def has_tests():
    return any(f.endswith("_test.go") for f in go_files())


def get_project_file_listing():
    rel = sorted(os.path.relpath(f, PROJECT_ROOT) for f in go_files())
    return "\n".join(f"  {r}" for r in rel) if rel else "  (no .go files yet)"
...
def write_gate_met():
    """True if the write phase produced enough. Manifest if present, else
    'at least one non-test .go file exists'."""
    if EXPECTED_FILES:
    ...
    n = len(go_files(include_tests=False))
    print(f"  {n} non-test .go file(s) present.")
    return n >= 1
...
def normalize_go():
    """Safe, deterministic pre-referee fixes: gofmt formatting + go mod tidy
    (repairs go.mod/go.sum for imports Larry added/removed). goimports is not
    installed, so missing-import errors are left to the toolchain + model."""
    subprocess.run(["gofmt", "-w", "."], cwd=PROJECT_ROOT,
                   capture_output=True, text=True)
    r = subprocess.run(["go", "mod", "tidy"], cwd=PROJECT_ROOT,
                       capture_output=True, text=True)
    if r.returncode == 0:
        print("  gofmt + go mod tidy ok")
    else:
        # tidy can legitimately fail while the code is broken; not fatal.
        print("  gofmt done; go mod tidy deferred (code not yet buildable)")
...
def _go(args):
    r = subprocess.run(["go"] + args, cwd=PROJECT_ROOT,
                       capture_output=True, text=True)
    return (r.stdout + r.stderr).strip(), r.returncode == 0


def run_referee():
    """go build ./... ; if ok, go vet ./... ; if tests exist, go test ./...
    Returns (report_text, passed, score). Lower score = better; build failure
    dominates, then vet, then test."""
    print("\n--- Go referee ---")
    normalize_go()

    build_out, build_ok = _go(["build", "./..."])
    print(f"  go build: {'PASS' if build_ok else 'FAIL'}")
    vet_out, vet_ok = "", True
    test_out, test_ok, tested = "", True, False

    if build_ok:
        vet_out, vet_ok = _go(["vet", "./..."])
        print(f"  go vet:   {'PASS' if vet_ok else 'FAIL'}")
        if has_tests():
            tested = True
            test_out, test_ok = _go(["test", "./..."])
            print(f"  go test:  {'PASS' if test_ok else 'FAIL'}")

    build_errs = len(re.findall(r"^\S+\.go:\d+:", build_out, re.M)) or (0 if build_ok else 1)
    vet_errs   = len(re.findall(r"^\S+\.go:\d+:", vet_out, re.M)) or (0 if vet_ok else 1)
    test_fails = 0 if test_ok else (test_out.count("--- FAIL") or 1)
    score = build_errs * 1000 + vet_errs * 100 + test_fails

    passed = build_ok and vet_ok and test_ok
    parts = [f"BUILD: {'PASS' if build_ok else 'FAIL'}", build_out]
    if build_ok:
        parts += [f"\nVET: {'PASS' if vet_ok else 'FAIL'}", vet_out]
        if tested:
...
def truncate_referee(text, max_lines=25):
    """Dedupe/cap toolchain output so it doesn't blow the model context."""
    seen, order = {}, []
    for line in text.splitlines():
        s = line.strip()
        if not s:
            continue
        key = re.sub(r"^\S+\.go:\d+:\d*:?\s*", "", s)  # strip file:line:col
        if key not in seen:
            seen[key] = s
            order.append(key)
...
def restore_src(snap):
    """Restore the EXACT snapshot tree. Same fix as run-build.py: rewriting only the
    captured files left a regressing round's NEW files behind, so the tree became
    "best files + the bad round's additions" — which is not the state keep-best
    promises. Remove out-of-snapshot files in scope first, then restore contents."""
...
def build_fix_msg(findings):
    ...
    return (
        f"Project root: {PROJECT_ROOT}/ (Go module)\n\n"
        f"Current .go files:\n{listing}\n\n"
        f"---\n\n"
        f"The Go toolchain reported these problems that must be fixed:\n\n"
        f"{findings}\n\n"
        f"---\n\n"
        f"Instructions:\n"
        f"- Read the affected file(s) with the read tool and fix ONLY the listed issues, "
        f"then write them back with the write/edit tool.\n"
        f"- 'go build' errors are compile failures; 'vet' are correctness warnings; "
        f"'--- FAIL' lines are failing tests.\n"
        f"- Unused imports/variables are hard compile errors in Go — remove them.\n"
        f"- Do NOT delete files. Do NOT change files that have no issues listed. "
        f"Do NOT add features beyond the fix.\n"
        f"After editing, the pipeline re-runs go build/vet/test."
    )
...
def main():
    ...
    if not os.path.isdir(PROJECT_ROOT):
        ...
        sys.exit(1)
    if not os.path.exists(PROJECT_ROOT + "/go.mod"):
        print(f"ERROR: no go.mod in {PROJECT_ROOT} (run 'gow new' or 'go mod init' first).")
        sys.exit(1)
    ...
    # Mid-tier arming — disarmed unless the policy permits the openrouter target,
    # and requires ESCALATE_MID_AFTER < ESCALATE_AFTER (else the ladder inverts).
    ...
    if not skip_larry and not review_only:
        ...
        for attempt in range(1, MAX_WRITE_ATTEMPTS + 1):
            ...
            if write_gate_met():
                ok = True
                break
            if wedged_write(len([f for f in EXPECTED_FILES if os.path.exists(f)]) if EXPECTED_FILES else len(go_files(include_tests=False)), coder.LAST["timed_out"], attempt):
    ...
    if review_peer and best["passed"]:
        ...
        try:
            ...
            files = [(os.path.relpath(f, PROJECT_ROOT), open(f).read())
                     for f in go_files(include_tests=True)]   # incl. tests so the reviewer sees them
            ...
            if clean:
                ...
                fixed = False
                # backends to try, in order: N pi rounds, then Claude if armed+available
